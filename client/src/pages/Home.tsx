...
export default function Home() {
  ...
  const timeLabel = currentTime.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  const pageSize = 20;
  ...
  return <div className="min-h-screen bg-[#101b2d] text-[#f9f0dd]">
    <header className="site-header"><div className="header-inner"><div className="logo-lockup"><img src="/manus-storage/logoesquina_6c4420b9.jpg" alt="Esquina da Carne" /><div><span>ESQUINA DA CARNE</span><small>Aracaju · Sergipe</small></div></div><div className="header-actions"><time className="digital-clock" dateTime={currentTime.toISOString()} aria-label={`Horário atual: ${timeLabel}`}>{timeLabel}</time><div className="qr-wrap"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=320x320&margin=8&data=${encodeURIComponent(`https://wa.me/${whatsapp}`)}`} alt="QR Code para pedidos no WhatsApp" /><span>Aponte a câmera<br />e peça pelo WhatsApp</span></div><button className="maintenance-icon" title="Manutenção dos preços" aria-label="Manutenção dos preços" onClick={() => setMaintenanceOpen(true)}><Settings2 className="size-5" /></button></div></div></header>
